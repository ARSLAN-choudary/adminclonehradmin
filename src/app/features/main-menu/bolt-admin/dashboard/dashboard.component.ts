...
// tip: refresh a bit earlier than 600s expiry if this is for the token
...
export class DashboardComponent {
  ...
  clientId = "vNkatedb03wF68AyMokEU";
  secretKey =
  ...
  constructor(private backend: BackendService, private destroyRef: DestroyRef) {
    // start immediately, then every ~9.5 minutes

    timer(0, REFRESH_MS)
      .pipe(
        switchMap(() =>
          this.backend
            .getBoltTokenFromServer(this.clientId, this.secretKey)
            .pipe(
              tap((res) => {
      ...
      .subscribe(() => this.lastRefreshed.set(new Date()));
  }
}
