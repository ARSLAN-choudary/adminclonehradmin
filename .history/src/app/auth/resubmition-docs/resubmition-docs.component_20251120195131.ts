...
export class ResubmitionDocsComponent {
  ...
  isLoading = true;
  staticUserId = "691f1eb4238c5fb7c1d8039d"; // Your static user ID
    ...
    if (this.hasUpdatedDocuments) {
      ...
      Object.keys(updatedDocuments).forEach(key => {
        ...
        if (doc.dataUrl) {
          ...
          formData.append('documentKeys', key); // Send which documents are being updated
        }
      });

      // Call your backend API to update documents
      this.backend.updateApplicationDocuments(this.staticUserId, formData).subscribe({
        next: (response) => {
