...
export class ManageUsersComponent implements OnInit, OnDestroy {
  ...
  openDocs(user: any) {
    ...
    // ADDED: Fetch user details from API
    this.fetchUserDetails(user._id);
  ...
  // ADDED: Method to fetch user details
  fetchUserDetails(userId: string) {
    this.backendService.userDetail(userId).subscribe({
      next: (response: any) => {
        if (response.status === 'success' && response.data) {
          this.userDetailData = response.data;
          this.formatUserDataForDisplay();
        }
      },
      error: (error) => {
  ...
  // ADDED: Method to format user data for display
    ...
    this.docData = [
      {
        ...
        fields: [
          { label: 'Full Name (English)', value: this.userDetailData.userNameEnglish + ' ' + this.userDetailData.surnameEnglish || 'N/A' },
          { label: 'Full Name (Georgian)', value: this.userDetailData.userNameGeorgian + ' ' + this.userDetailData.surnameGeorgian || 'N/A' },
          { label: 'Email', value: this.userDetailData.email || 'N/A' },
          { label: 'Phone', value: this.userDetailData.phone || 'N/A' },
          { label: 'Date of Birth', value: this.userDetailData.dateOfBirth || 'N/A' },
          { label: 'Gender', value: this.userDetailData.gender || 'N/A' },
          { label: 'Marital Status', value: this.userDetailData.martialStatus || 'N/A' },
          { label: 'Citizenship', value: this.userDetailData.citizenship || 'N/A' },
          { label: 'Document Type', value: this.userDetailData.documentType || 'N/A' },
          { label: 'Document Number', value: this.userDetailData.documentNumber || 'N/A' }
        ]
      },
      {
        section: 'Address Information',
        fields: [
          { label: 'Legal Address', value: this.userDetailData.legalAdress || 'N/A' },
          { label: 'Location', value: this.userDetailData.location || 'N/A' }
        ]
      },
      {
        section: 'Education',
        fields: this.formatEducationData()
      },
      {
        section: 'Work Experience',
        fields: this.formatWorkExperienceData()
      },
      {
        section: 'Languages',
        fields: this.formatLanguagesData()
      },
      {
        section: 'Skills',
        fields: this.formatSkillsData()
      },
      {
        section: 'Bank Details',
        fields: [
          { label: 'Bank Name', value: this.userDetailData.bankName || 'N/A' },
          { label: 'Account Number', value: this.userDetailData.accountNumber || 'N/A' },
          { label: 'Account Holder', value: this.userDetailData.accountHolderName || 'N/A' }
        ]
      },
      {
        section: 'Emergency Contact',
        fields: [
          { label: 'Full Name', value: this.userDetailData.emergencyFullName || 'N/A' },
          { label: 'Relationship', value: this.userDetailData.emergencyRelationship || 'N/A' },
          { label: 'Address', value: this.userDetailData.emergencyAddress || 'N/A' },
          { label: 'Contact Number', value: this.userDetailData.emergencyContactNumber || 'N/A' },
          { label: 'Notes', value: this.userDetailData.emergencyNotes || 'N/A' }
        ]
      },
      {
        section: 'Work Authorization',
        fields: [
          { label: 'Allowed to Work', value: this.userDetailData.allowedToWork ? 'Yes' : 'No' },
          { label: 'Remarks', value: this.userDetailData.rightToWorkRemarks || 'N/A' }
        ]
      },
      {
        section: 'Additional Information',
        fields: [
          { label: 'Additional Notes', value: this.userDetailData.additionalNotes || 'N/A' }
        ]
      }
    ];
  }

  // ADDED: Helper methods to format arrays
  formatEducationData(): any[] {
    if (!this.userDetailData.education || !Array.isArray(this.userDetailData.education)) {
      return [{ label: 'Education', value: 'No education data available' }];
    }

    return this.userDetailData.education.map((edu: any, index: number) => [
      { label: `Education ${index + 1} - Institution`, value: edu.institution || 'N/A' },
      { label: `Education ${index + 1} - Qualification`, value: edu.qualification || 'N/A' },
      { label: `Education ${index + 1} - Period`, value: `${edu.from || 'N/A'} to ${edu.to || 'N/A'}` },
      { label: `Education ${index + 1} - Currently Studying`, value: edu.currentlyStudying ? 'Yes' : 'No' },
      { label: `Education ${index + 1} - Notes`, value: edu.notes || 'N/A' }
    ]).flat();
  }

  formatWorkExperienceData(): any[] {
    if (!this.userDetailData.workExperience || !Array.isArray(this.userDetailData.workExperience)) {
      return [{ label: 'Work Experience', value: 'No work experience data available' }];
    }

    return this.userDetailData.workExperience.map((work: any, index: number) => [
      { label: `Work ${index + 1} - Company`, value: work.companyName || 'N/A' },
      { label: `Work ${index + 1} - Job Title`, value: work.jobTitle || 'N/A' },
      { label: `Work ${index + 1} - Location`, value: work.cityCountry || 'N/A' },
      { label: `Work ${index + 1} - Period`, value: `${work.from || 'N/A'} to ${work.to || 'N/A'}` },
      { label: `Work ${index + 1} - Still Working`, value: work.stillWorking ? 'Yes' : 'No' },
      { label: `Work ${index + 1} - Salary`, value: work.grossSalary || 'N/A' },
      { label: `Work ${index + 1} - Reason for Leaving`, value: work.reasonForLeaving || 'N/A' },
      { label: `Work ${index + 1} - Notes`, value: work.notes || 'N/A' }
    ]).flat();
  }

  formatLanguagesData(): any[] {
    if (!this.userDetailData.languages || !Array.isArray(this.userDetailData.languages)) {
      return [{ label: 'Languages', value: 'No language data available' }];
    }

    return this.userDetailData.languages.map((lang: any, index: number) => [
      { label: `Language ${index + 1}`, value: `${lang.language || 'N/A'} - ${lang.level || 'N/A'}` }
    ]).flat();
  }

  formatSkillsData(): any[] {
    const skills = [];

    if (this.userDetailData.computerSkills && Array.isArray(this.userDetailData.computerSkills)) {
      skills.push({ label: 'Computer Skills', value: this.userDetailData.computerSkills.join(', ') || 'N/A' });
    }

    if (this.userDetailData.administrativeSkills && Array.isArray(this.userDetailData.administrativeSkills)) {
      skills.push({ label: 'Administrative Skills', value: this.userDetailData.administrativeSkills.join(', ') || 'N/A' });
    }

    if (this.userDetailData.otherSkills) {
      skills.push({ label: 'Other Skills', value: this.userDetailData.otherSkills || 'N/A' });
    }

    if (this.userDetailData.skillRating) {
      skills.push({ label: 'Skill Rating', value: this.userDetailData.skillRating || 'N/A' });
    }

    return skills.length > 0 ? skills : [{ label: 'Skills', value: 'No skills data available' }];
  }

  // Your existing closeDocs method remains exactly the same
  closeDocs() {
    if (document.activeElement instanceof HTMLElement) {
      document.activeElement.blur();
    }
